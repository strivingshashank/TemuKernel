...
echo "[1.5] Assembling Temu entry point..."
nasm -f bin "$SRC_DIR/entry.asm" -o "$TEMU_BIN" -I"$SRC_DIR/"
...
# Append temu right after it (starting from sector 1)
dd if="$BIN_DIR/temu.bin" of="$TEMU_IMG" bs=512 seek=1 conv=notrunc
