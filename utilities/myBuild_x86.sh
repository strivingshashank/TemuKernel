...
FLOPPY_IMG="$BIN_DIR/bootFloppy.img"
...
dd if="$BIN_DIR/bootloader.bin" of="$FLOPPY_IMG" bs=512 count=1 conv=notrunc status=none

# Append kernel right after it (starting from sector 1)
dd if="$BIN_DIR/kernel.bin" of="$FLOPPY_IMG" bs=512 seek=1 conv=notrunc status=none
